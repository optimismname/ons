...
contract('ReverseRegistar', function (accounts) {
    ...
    let node;
    let registrar, resolver, ons;

    beforeEach(async () => {
        node = namehash.hash(accounts[0].slice(2).toLowerCase() + ".addr.reverse");
        ons = await ONS.new();
        resolver = await DummyResolver.new();
        registrar = await ReverseRegistrar.new(ons.address, resolver.address);

        await ons.setSubnodeOwner('0x0', sha3('reverse'), accounts[0], {from: accounts[0]});
        await ons.setSubnodeOwner(namehash.hash('reverse'), sha3('addr'), registrar.address, {from: accounts[0]});
    });
    ...
    it('allows an account to specify resolver', async () =>  {
        ...
        assert.equal(await ons.owner(node), accounts[1]);
        assert.equal(await ons.resolver(node), accounts[2]);
    });
    ...
    it('does not overwrite resolver if not specified', async () => {
        ...
        assert.equal(await ons.owner(node), accounts[3]);
        assert.equal(await ons.resolver(node), accounts[2]);
    });

    it('sets name records', async () => {
        await registrar.setName('testname', {from: accounts[0], gas: 1000000});
        assert.equal(await ons.resolver(node), resolver.address);
